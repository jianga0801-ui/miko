...
const repo = process.env.GH_REPO ?? "jianga0801-ui/miko"
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/miko packages/sdk packages/plugin packages/desktop sdks/vscode packages/extensions github`.text()
