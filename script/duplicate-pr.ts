...
import { createMiko } from "@miko-ai/sdk"
...
`)
  ...
  const miko = await createMiko({ port: 0 })
  ...
  try {
    ...
    const session = await miko.client.session.create()
    const result = await miko.client.session
      .prompt({
