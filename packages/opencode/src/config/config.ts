...
function globalConfigFile() {
  const candidates = ["miko.jsonc", "miko.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".miko") || dir.endsWith(".opencode") || dir === Flag.OPENCODE_CONFIG_DIR) {
            const files = dir.endsWith(".miko") ? ["miko.json", "miko.jsonc"] : ["opencode.json", "opencode.jsonc"]
        ...
        if (existsSync(managedDir)) {
          for (const file of ["miko.json", "miko.jsonc", "opencode.json", "opencode.jsonc"]) {
