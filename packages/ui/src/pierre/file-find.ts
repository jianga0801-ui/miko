...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("miko-find")
  api.delete("miko-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("miko-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("miko-find", new Highlight(...rest))
    return true
  }
