...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@miko/acme")).toBe("@miko/acme")
    expect(Npm.sanitize("@miko/acme@1.0.0")).toBe("@miko/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/miko/acme.git"
    const expected = win ? "acme@git+https_//github.com/miko/acme.git" : spec
