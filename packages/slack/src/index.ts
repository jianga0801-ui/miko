...
import { createMiko, type ToolPart } from "@miko-ai/sdk"
...
console.log("🚀 Starting miko server...")
const miko = await createMiko({
  port: 0,
})
console.log("✅ Miko server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await miko.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new miko session...")
    const { client, server } = miko
    ...
    console.log("✅ Created miko session:", createResult.data.id)
  ...
  console.log("📝 Sending to miko:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Miko response:", JSON.stringify(result, null, 2))
