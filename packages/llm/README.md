...
Schema-first LLM core for miko. One typed request, response, event, and tool language; provider quirks live in adapters, not in calling code.

```ts
import { Effect } from "effect"
import { LLM, LLMClient } from "@miko-ai/llm"
...
```ts
import { OpenAI, CloudflareAIGateway } from "@miko-ai/llm/providers"
