...
describe("miko read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // MIKO_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ miko }) =>
      Effect.gen(function* () {
        const r = yield* miko.spawn(["mcp", "list"])
        miko.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ miko }) =>
      Effect.gen(function* () {
        const r = yield* miko.spawn(["providers", "list"])
        miko.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ miko }) =>
      Effect.gen(function* () {
        const r = yield* miko.spawn(["models"])
        miko.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ miko }) =>
      Effect.gen(function* () {
        const r = yield* miko.spawn(["agent", "list"])
        miko.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh MIKO_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ miko }) =>
      Effect.gen(function* () {
        const r = yield* miko.spawn(["session", "list"])
        miko.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ miko }) =>
      Effect.gen(function* () {
        const r = yield* miko.spawn(["stats"])
        miko.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under MIKO_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ miko }) =>
      Effect.gen(function* () {
        const r = yield* miko.spawn(["db", "path"])
        miko.expectExit(r, 0, "db path")
