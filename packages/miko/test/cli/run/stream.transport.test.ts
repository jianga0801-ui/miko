...
import { MikoClient, type GlobalEvent } from "@miko-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<MikoClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<MikoClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<MikoClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<MikoClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<MikoClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://miko.test"),
...
function sdk(
  input: {
    ...
    subscribe?: MikoClient["event"]["subscribe"]
    globalEvent?: MikoClient["global"]["event"]
    promptAsync?: MikoClient["session"]["promptAsync"]
    status?: MikoClient["session"]["status"]
    messages?: MikoClient["session"]["messages"]
    children?: MikoClient["session"]["children"]
    permissions?: MikoClient["permission"]["list"]
    questions?: MikoClient["question"]["list"]
  } = {},
) {
  const client = new MikoClient()

  const subscribe: MikoClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: MikoClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: MikoClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: MikoClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: MikoClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: MikoClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: MikoClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: MikoClient["question"]["list"] = input.questions ?? (() => ok([]))
