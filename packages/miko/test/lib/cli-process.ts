// Subprocess test harness for the miko CLI. Spawns the real binary against
...
//
// Configuration flows through miko's built-in test affordances:
//   - MIKO_CONFIG_CONTENT      : provider config inline, no files to find
//   - MIKO_TEST_HOME           : pins os.homedir() → tmpdir
//   - MIKO_DISABLE_PROJECT_CONFIG : skip walking up for miko.json
//   - MIKO_PURE                : skip external plugin discovery + install
//   - MIKO_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `miko.run` is fully wired. The shape supports adding more
// builders (`miko.serve(opts)`, `miko.acp(opts)`, `miko.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of MikoCli.
import { test, type TestOptions } from "bun:test"
import { AppFileSystem } from "@miko-ai/core/filesystem"
import { AppProcess } from "@miko-ai/core/process"
...
const mikoRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(mikoRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    MIKO_CONFIG_CONTENT: configJson,
    MIKO_DISABLE_PROJECT_CONFIG: "1",
    MIKO_PURE: "1",
    MIKO_DISABLE_AUTOUPDATE: "1",
    MIKO_DISABLE_AUTOCOMPACT: "1",
    MIKO_DISABLE_MODELS_FETCH: "1",
    MIKO_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `miko run`. New flags should
...
// `miko serve` is a long-lived process — it never exits on its own.
...
// `miko acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type MikoCli = {
  ...
  // Spawn `miko serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `miko acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("miko.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("miko.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "miko server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `miko serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("miko.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const miko: MikoCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, miko })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `miko.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "miko") {
...
// every call site. Use this for any test that needs the miko CLI fixture.
//
...
// resources (e.g. `miko.serve`) without an extra `Effect.scoped` wrapper —
