...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.MIKO_WEBSEARCH_PROVIDER

    try {
      process.env.MIKO_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.MIKO_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.MIKO_WEBSEARCH_PROVIDER
      else process.env.MIKO_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for miko or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderV2.ID.miko, { exa: false, parallel: false })).toBe(true)
  ...
  test("resolveTavilyKey prefers env, then the stored api key", () => {
    const original = process.env.TAVILY_API_KEY
    try {
      delete process.env.TAVILY_API_KEY
      expect(resolveTavilyKey(undefined)).toBeUndefined()
      expect(resolveTavilyKey({ type: "oauth" })).toBeUndefined()
      expect(resolveTavilyKey({ type: "api", key: "tvly-stored" })).toBe("tvly-stored")

      process.env.TAVILY_API_KEY = "tvly-env"
      expect(resolveTavilyKey({ type: "api", key: "tvly-stored" })).toBe("tvly-env")
    } finally {
