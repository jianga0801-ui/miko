...
import { ProjectTable } from "@miko-ai/core/project/sql"
import { Session as SessionNs } from "@/session/session"
import { SessionID } from "@/session/schema"
import { SessionTable } from "@miko-ai/core/session/sql"
import { EventSequenceTable } from "@miko-ai/core/event/sql"
...
import { WorkspaceV2 } from "@miko-ai/core/workspace"
import { WorkspaceTable } from "@miko-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  MIKO_AUTH_CONTENT: process.env.MIKO_AUTH_CONTENT,
  MIKO_EXPERIMENTAL_WORKSPACES: process.env.MIKO_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.MIKO_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@miko.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.MIKO_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=miko-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.MIKO_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.MIKO_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.MIKO_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=miko-test")
