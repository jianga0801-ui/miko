...
import { PermissionTable, SessionTable } from "@miko-ai/core/session/sql"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const mikoFile = path.join(tmp, ".git", "miko")
      expect(yield* Effect.promise(() => Bun.file(mikoFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "miko")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "miko")
      const wrongCache = path.join(parentDir, ".git", "miko")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "miko")
      const cacheB = path.join(bareB, "miko")
      const wrongCache = path.join(parentDir, ".git", "miko")
