...
import { ModelsDev } from "@miko-ai/core/models-dev"
import { LocationServiceMap } from "@miko-ai/core/location-layer"
import { HttpRecorder, Redactor } from "@miko-ai/http-recorder"
...
import { LLMEvent, LLMResponse } from "@miko-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@miko-ai/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.miko.dev/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.MIKO_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["miko", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("MIKO_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("MIKO_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["miko", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "miko-proxy",
    name: "Miko proxy",
    providerID: ProviderV2.ID.miko,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["miko", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.MIKO_RECORD_CONSOLE_TOKEN && process.env.MIKO_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.miko,
        name: "Miko Zen",
        env: ["MIKO_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.MIKO_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.MIKO_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.MIKO_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["miko", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("MIKO_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("MIKO_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("MIKO_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set MIKO_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the miko LLM stack remain real.
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "miko.json"),
      JSON.stringify({ $schema: "https://miko.dev/config.json", ...scenario.config(model) }),
    ),
  )
