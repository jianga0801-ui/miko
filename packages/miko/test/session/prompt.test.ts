...
import { SessionMessageTable } from "@miko-ai/core/session/sql"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<Config.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://miko.dev/config.json", ...config }),
  )
})
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/miko/src/session/processor.ts")
