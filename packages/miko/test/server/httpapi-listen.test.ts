...
const original = {
  ...
  envPassword: process.env.MIKO_SERVER_PASSWORD,
  envUsername: process.env.MIKO_SERVER_USERNAME,
}
const auth = { username: "miko", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.MIKO_SERVER_PASSWORD = original.MIKO_SERVER_PASSWORD
  Flag.MIKO_SERVER_USERNAME = original.MIKO_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.MIKO_SERVER_PASSWORD
  else process.env.MIKO_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.MIKO_SERVER_USERNAME
  else process.env.MIKO_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.MIKO_SERVER_PASSWORD = auth.password
  Flag.MIKO_SERVER_USERNAME = auth.username
  process.env.MIKO_SERVER_PASSWORD = auth.password
  process.env.MIKO_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.MIKO_SERVER_USERNAME = auth.username
  delete process.env.MIKO_SERVER_PASSWORD
  process.env.MIKO_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-miko-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-miko-ticket": "1" }),
  ...
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-miko-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-miko-ticket": "1" },
      })
