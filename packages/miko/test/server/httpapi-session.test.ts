...
import { SessionMessageTable, SessionTable } from "@miko-ai/core/session/sql"
import { SessionMessage } from "@miko-ai/core/session/message"
...
const originalWorkspaces = Flag.MIKO_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.MIKO_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-miko-directory": test.directory }
        ...
        const prompt = yield* request(pathFor(SessionPaths.prompt, { sessionID: missingSession }), {
          ...
          method: "POST",
          body: JSON.stringify({ agent: "miko", noReply: true, parts: [{ type: "text", text: "hello" }] }),
        })
    ...
    () =>
      Effect.gen(function* () {
        ...
        const messages = yield* request(`/api/session/${session.id}/message`, {
          headers: { "x-miko-directory": test.directory },
        })
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const headers = { "x-miko-directory": test.directory, "content-type": "application/json" }
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        Flag.MIKO_EXPERIMENTAL_WORKSPACES = true
        ...
        const created = yield* requestJson<Session.Info>(`${SessionPaths.create}?workspace=${workspace.id}`, {
          method: "POST",
          headers: { "x-miko-directory": test.directory, "content-type": "application/json" },
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const currentDir = path.join(test.directory, "packages", "miko", "src")
