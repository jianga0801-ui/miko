...
type Sdk = ReturnType<typeof createMikoClient>
...
function serverFetch(serverPath: ServerPath, input?: { password?: string; username?: string }) {
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.MIKO_SERVER_PASSWORD = input?.password
      Flag.MIKO_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return AppFileSystem.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".miko", "skills", "project-rest-skill", "SKILL.md"),
      `---
