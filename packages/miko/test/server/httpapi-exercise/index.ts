 ...
 *
 * The script intentionally isolates `MIKO_DB` before importing modules that touch
...
void (await import("@miko-ai/core/util/log")).init({ print: false })
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "miko.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "miko.jsonc")).text(),
          )
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-miko-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-miko-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "miko", model: "test" })}`,
      headers: ctx.headers(),
    }))
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: ctx.headers(),
      body: { agent: "miko", model: { providerID: "test", modelID: "test-model" }, command: "printf shell-ok" },
    }))
    .json(
      200,
      (body) => {
