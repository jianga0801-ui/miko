...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "miko.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "miko.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "miko.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "miko.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["miko.json", "miko.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "miko.json"),
        path.join(project, "miko.jsonc"),
      ])
    })
  })
