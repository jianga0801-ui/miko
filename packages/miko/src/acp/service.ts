...
import type { Message, MikoClient, SessionMessageResponse } from "@miko-ai/sdk/v2"
...
export const AuthMethodID = "miko-login"
...
export class Service extends Context.Service<Service, Interface>()("@miko/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `miko auth login` in the terminal",
      name: "Login with miko",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: MikoClient) {
...
function makeUsageService(sdk: MikoClient) {
...
async function loadDirectorySnapshot(sdk: MikoClient, directory: string) {
  return ACPProfile.measure("acp.directory.load", async () => {
    ...
    return Directory.build({
      ...
      providers,
      modes,
      defaultModeID: agents.find((agent) => agent.mode === "primary" && agent.hidden !== true)?.name ?? "miko",
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, miko provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const mikoProvider = providers[ProviderV2.ID.make("miko")]
  const mikoModel = mikoProvider ? Provider.sort(Object.values(mikoProvider.models))[0] : undefined
  if (mikoProvider && mikoModel) return { providerID: mikoProvider.id, modelID: mikoModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Miko service failure", service })
}
