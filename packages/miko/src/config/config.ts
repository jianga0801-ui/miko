...
import { InstallationLocal, InstallationVersion } from "@miko-ai/core/installation/version"
...
import { NonNegativeInt, PositiveInt, type DeepMutable } from "@miko-ai/core/schema"
...
import { ConfigExperimental } from "@miko-ai/core/config/experimental"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in miko config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for miko serve and web commands",
  }),
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://miko.dev/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://miko.dev/docs/agents" }),
...
// Uses the shared `DeepMutable` from `@miko-ai/core/schema`. See the definition
...
export class Service extends Context.Service<Service, Interface>()("@miko/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["miko.jsonc", "miko.json", "miko.jsonc", "miko.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.MIKO_CONFIG && !Flag.MIKO_CONFIG_DIR && !Flag.MIKO_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "miko.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "miko.jsonc"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "miko.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "miko.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "MIKO_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/miko`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://miko.dev/config.json"
        ...
        if (Flag.MIKO_CONFIG) {
          yield* merge(Flag.MIKO_CONFIG, yield* loadFile(Flag.MIKO_CONFIG, authEnv))
          log.debug("loaded custom config", { path: Flag.MIKO_CONFIG })
        }

        if (!Flag.MIKO_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("miko", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.MIKO_CONFIG_DIR) {
          log.debug("loading config from MIKO_CONFIG_DIR", { path: Flag.MIKO_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".miko") || dir.endsWith(".miko") || dir === Flag.MIKO_CONFIG_DIR) {
            const files = dir.endsWith(".miko") ? ["miko.json", "miko.jsonc"] : ["miko.json", "miko.jsonc"]
          ...
          // Auto-discovered plugins under `.miko/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.MIKO_CONFIG_CONTENT) {
          const source = "MIKO_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.MIKO_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from MIKO_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["MIKO_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("MIKO_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (existsSync(managedDir)) {
          for (const file of ["miko.json", "miko.jsonc", "miko.json", "miko.jsonc"]) {
        ...
        if (Flag.MIKO_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.MIKO_PERMISSION))
          } catch (err) {
            log.warn("MIKO_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.MIKO_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.MIKO_DISABLE_PRUNE) {
