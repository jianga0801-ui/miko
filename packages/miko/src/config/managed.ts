...
const MANAGED_PLIST_DOMAIN = "ai.miko.managed"

// Keys injected by macOS/MDM into the managed plist that are not Miko config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/miko"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "miko")
    default:
      return "/etc/miko"
  }
}
...
export function managedConfigDir() {
  return process.env.MIKO_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
