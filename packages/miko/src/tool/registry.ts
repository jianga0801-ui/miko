...
import { MimoAnalyzeMediaTool } from "./mimo_analyze_media"
import { WriteTool } from "./write"
import { InvalidTool } from "./invalid"
import { SkillTool } from "./skill"
import * as Tool from "./tool"
import { Config } from "@/config/config"
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@miko-ai/plugin"
...
import { WebSearchTool, resolveTavilyKey } from "./websearch"
...
export function webSearchEnabled(
  ...
  return providerID === ProviderV2.ID.miko || !!flags.exa || !!flags.parallel || !!flags.tavily
}
...
export class Service extends Context.Service<Service, Interface>()("@miko/ToolRegistry") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const mimoAnalyzeMedia = yield* MimoAnalyzeMediaTool
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const tool = yield* Effect.all({
          ...
          mimo_analyze_media: Tool.init(mimoAnalyzeMedia),
          todo: Tool.init(todo),
    ...
    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      ...
      const tavilyEnabled = Boolean(resolveTavilyKey(storedTavily))
