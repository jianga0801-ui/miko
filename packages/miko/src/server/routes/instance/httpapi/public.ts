...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for miko serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(MikoHttpApi)
...
export const PublicApi = MikoHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "miko",
    version: "1.0.0",
    description: "miko api",
