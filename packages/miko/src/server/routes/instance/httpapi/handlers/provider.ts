...
import { filterMimoProviders, isMimoProviderID } from "@/provider/mimo-setup"
  ...
  Effect.gen(function* () {
    ...
    const list = Effect.fn("ProviderHttpApi.list")(function* () {
      ...
      for (const [key, value] of Object.entries(all)) {
        if (!isMimoProviderID(key)) continue
        if ((enabled ? enabled.has(key) : true) && !disabled.has(key)) filtered[key] = value
      }
      const connected = filterMimoProviders(yield* provider.list())
