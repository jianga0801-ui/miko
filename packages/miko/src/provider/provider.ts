...
import { optionalOmitUndefined } from "@miko-ai/core/schema"
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  "@ai-sdk/github-copilot": () =>
    import("@miko-ai/core/github-copilot/copilot-provider").then((m) => m.createOpenaiCompatible),
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  const mimoOpenAICompatible = () =>
    Effect.succeed({
      autoload: false,
      options: {
        fetch: makeMimoFetch(globalThis.fetch, { webSearch: resolveMimoWebSearchConfig() }),
      },
    })
  ...
  return {
    ...
    miko: Effect.fnUntraced(function* (input: Info) {
      ...
      const ok =
        ...
        Boolean((yield* dep.config()).provider?.["miko"]?.options?.apiKey)
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from miko.json provider config
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `miko/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
      ...
      return {
        ...
        options: {
          apiKey,
          headers: {
            "User-Agent": `miko/${InstallationVersion} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        },
        async getModel(sdk: any, modelID: string) {
          return sdk.languageModel(modelID)
    ...
    "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `miko auth cloudflare-ai-gateway`.",
        )
      }
      ...
      const opts = {
        ...
        headers: {
          "User-Agent": `miko/${InstallationVersion} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
        },
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "miko",
          },
        },
      }),
    kilo: () =>
    ...
    xiaomi: mimoOpenAICompatible,
    "xiaomi-token-plan-cn": mimoOpenAICompatible,
    "xiaomi-token-plan-sgp": mimoOpenAICompatible,
    "xiaomi-token-plan-ams": mimoOpenAICompatible,
  }
}
...
export class Service extends Context.Service<Service, Interface>()("@miko/Provider") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        const catalog = filterMimoProviders(mapValues(modelsDev, fromModelsDevProvider))
        ...
        function isProviderAllowed(providerID: ProviderV2.ID): boolean {
          if (!isMimoProviderID(providerID)) return false
        ...
        for (const [providerID, provider] of configProviders) {
          if (!isMimoProviderID(providerID)) continue
