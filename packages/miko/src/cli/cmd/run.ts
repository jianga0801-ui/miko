...
//      miko server and runs interactive mode against it.
//
...
import { createMikoClient, type MikoClient, type ToolPart } from "@miko-ai/sdk/v2"
...
type ModelInput = Parameters<MikoClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run miko with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running miko server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to MIKO_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to MIKO_SERVER_USERNAME or 'miko')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: MikoClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: MikoClient, sessionID: string) {
      ...
      async function current(sdk: MikoClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: MikoClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: MikoClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: MikoClient) {
        ...
        async function loop(client: MikoClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createMikoClient({
        baseUrl: "http://miko.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
