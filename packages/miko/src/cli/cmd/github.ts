...
const AGENT_USERNAME = "miko-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/miko.yml"
    ...
    yield* Effect.promise(async () => {
      {
        ...
        async function installGitHubApp() {
          ...
          // Open browser
          const url = "https://github.com/apps/miko-agent"
          const command =
          ...
          async function getInstallation() {
            return await fetch(
              `https://api.miko.dev/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
            )
              .then((res) => res.json())
...
jobs:
  miko:
    if: |
      ...
      startsWith(github.event.comment.body, '/miko')
...
export const GithubRunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.github.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.miko.dev" : "https://miko.dev"
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.miko.dev"
        ...
        return runLocalEffect(
          Effect.gen(function* () {
            ...
            const result = yield* prompt.prompt({
              ...
              // agent is omitted - server will use default_agent from config or fall back to "miko"
              parts: [
                {
                  id: PartID.ascending(),
                  type: "text",
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("miko-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `miko/${type}-${hex}-${timestamp}`
        }
        return `miko/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/miko-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[miko session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the miko infrastructure after your response",
