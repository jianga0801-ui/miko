...
function footerLabels(input: Pick<RunInput, "agent" | "model" | "variant">): FooterLabels {
  ...
  const agentLabel = agent === "build" ? "Miko" : Locale.titlecase(agent)
...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "miko.agent.name": input.agent,
      "miko.directory": input.directory,
      "miko.first": input.first,
      "miko.model.provider": input.model?.providerID,
      "miko.model.id": input.model?.modelID,
      "miko.model.variant": input.variant,
