...
//
// Renders the full miko entry logo and a compact [O] exit badge, plus
...
function draw(
  ...
  input: {
    ...
    shadow: ColorInput
    attrs?: number
    themeColors?: { left: ColorInput; right: ColorInput }
  },
) {
  let x = input.left
  for (const cell of cells(row)) {
    ...
    if (input.themeColors) {
      const i = x - input.left
      const isM = input.left === 0 && i <= 4
      const isK = input.left > 0 && i <= 3
      const isGray = isM || isK
      cellFg = isGray ? input.themeColors.left : input.themeColors.right
    }

    if (cell.mark === "full" || cell.mark === "mix") {
      push(lines, x, input.top, cell.char, cellFg, input.shadow, input.attrs)
      x += 1
      continue
    }
    ...
    push(lines, x, input.top, cell.char, cellFg, undefined, input.attrs)
    x += 1
  }
}
