...
import { createMikoClient } from "@miko-ai/sdk/v2"
...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "miko.mode": input.resolveSession ? "local" : "attach",
      "miko.initial_input": !!input.initialInput,
      "miko.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "miko.directory": ctx.directory,
        "miko.resume": ctx.resume === true,
        "miko.agent.name": state.agent,
        "miko.model.provider": state.model?.providerID,
        "miko.model.id": state.model?.modelID,
        "miko.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "miko.model.provider": model.providerID,
            "miko.model.id": model.modelID,
            "miko.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "miko.model.id": state.model?.modelID,
                "miko.model.variant": state.activeVariant,
                "miko.prompt.chars": prompt.text.length,
                "miko.prompt.parts": prompt.parts.length,
                "miko.prompt.include_files": includeFiles,
                "miko.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "miko.directory": input.directory,
      "miko.initial_input": !!input.initialInput,
      "miko.demo": input.demo,
    },
    async () => {
