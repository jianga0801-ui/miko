...
const TIPS: Tip[] = [
  ...
  "Use {highlight}/ob{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}miko github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/ob fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/ob{/highlight} on PR code lines for targeted code reviews",
...
const TIPS_ZH: Tip[] = [
  ...
  "在 GitHub issue/PR 中使用 {highlight}/ob{/highlight} 触发 AI 操作",
  "运行 {highlight}miko github install{/highlight} 设置 GitHub 工作流",
  "在 issue 上评论 {highlight}/ob fix this{/highlight} 自动创建 PR",
  "在 PR 代码行上评论 {highlight}/ob{/highlight} 进行针对性的代码审查",
