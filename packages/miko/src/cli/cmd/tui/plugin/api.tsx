import type { TuiDialogSelectOption, TuiPluginApi, TuiRouteDefinition, TuiSlotProps } from "@miko-ai/plugin/tui"
...
import type { useMikoKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useMikoKeymap>
...
export function createTuiApi(input: Input): TuiPluginApi {
  ...
  return {
    ...
    mode: {
      current() {
        return Keymap.getMikoModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getMikoModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
