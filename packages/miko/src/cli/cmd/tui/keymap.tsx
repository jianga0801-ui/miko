...
export const MIKO_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const MIKO_MODE_KEY = "miko.mode"

export const MikoKeymapProvider = KeymapProvider
export const useMikoKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type MikoModeStack = ReturnType<typeof createMikoModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, MikoModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createMikoModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(MIKO_MODE_KEY, MIKO_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(MIKO_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(MIKO_MODE_KEY, stack.at(-1)?.mode ?? MIKO_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? MIKO_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(MIKO_MODE_KEY, undefined)
...
export function useMikoModeStack() {
  return getMikoModeStack(useMikoKeymap())
}

export function getMikoModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Miko mode stack is not registered for this keymap")
  return value
}
...
export function registerMikoKeymap(
  keymap: OpenTuiKeymap,
  renderer: CliRenderer,
  config: Pick<TuiConfig.Resolved, "keybinds" | "leader_timeout">,
) {
  const modeStack = createMikoModeStack(keymap)
