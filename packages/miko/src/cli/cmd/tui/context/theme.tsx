...
import miko from "./theme/miko.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@miko-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "miko")
        draft.active = typeof active === "string" ? active : "miko"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.miko, store.mode)
    })
