...
const TUI_SCHEMA_URL = "https://miko.dev/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from miko.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const miko = await mikoFiles(input)
  for (const file of miko) {
...
async function mikoFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "miko"),
    ...(await Filesystem.findUp(["miko.json", "miko.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "miko"))
  }
  if (Flag.MIKO_CONFIG) files.push(Flag.MIKO_CONFIG)
