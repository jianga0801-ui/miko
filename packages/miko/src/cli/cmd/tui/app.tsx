...
export function tui(input: TuiInput): TuiHandle {
  ...
  const unregisterKeymap = registerMikoKeymap(keymap, renderer, input.config)
...
async function mountTui(input: TuiInput & { keymap: ReturnType<typeof createDefaultOpenTuiKeymap>; exit: Exit }) {
  ...
  await render(() => {
    return (
      ...
      >
        <MikoKeymapProvider keymap={input.keymap}>
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.MIKO_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("Miko")
      return
    }
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`Miko | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`Miko | ${route.data.id}`)
    }
  })
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.MIKO_EXPERIMENTAL_WORKSPACES,
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to Miko v${result.data.version}. Please restart the application.`,
    )
