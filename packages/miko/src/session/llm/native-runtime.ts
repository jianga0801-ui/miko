...
import { tool as nativeTool, ToolFailure, type JsonSchema, type LLMEvent } from "@miko-ai/llm"
import type { LLMClientShape } from "@miko-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("miko"))
    return { type: "unsupported", reason: "provider is not openai, miko, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @miko-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains miko-owned. The native runtime only adapts
      // the @miko-ai/llm tool call back into the AI SDK Tool.execute shape.
      nativeTool({
