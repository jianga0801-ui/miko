...
async function resolveGitRoot(projectRoot: string): Promise<string | undefined> {
	...
	const exitCode = await withTimeout(proc.exited, 5000, "git rev-parse timed out").catch((e) => {
...
export async function getProjectId(projectRoot: string, client?: OpencodeClient): Promise<string> {
	...
	projectRoot = (await resolveGitRoot(projectRoot)) ?? projectRoot
	...
	const cacheFile = path.join(gitDir, "miko")
