...
import type { Event } from "@miko-ai/sdk"
...
async function forkWithContext(
	...
	try {
		const workspaceBase = path.join(os.homedir(), ".local", "share", "miko", "workspace")
		const delegationsBase = path.join(os.homedir(), ".local", "share", "miko", "delegations")
...
async function loadWorktreeConfig(directory: string, log: Logger): Promise<WorktreeConfig> {
	const configPath = path.join(directory, ".miko", "worktree.jsonc")
...
`
			...
			await mkdir(path.join(directory, ".miko"), { recursive: true })
...
export default { id: "worktree", server: WorktreePluginWithInternals }
