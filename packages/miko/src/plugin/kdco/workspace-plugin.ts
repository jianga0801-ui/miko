...
import { type Plugin, tool } from "@miko-ai/plugin"
import { z } from "zod"
...
const WorkspacePlugin: Plugin = async (ctx) => {
	...
	const baseDir = path.join(os.homedir(), ".local", "share", "miko", "workspace", projectId)
	...
	return {
		...
		// Universal date awareness injection.
		// NOTE: per-agent PLAN/BUILD rules are applied via the plan/build agent
		// `system` prompts in miko.jsonc, because miko's
		// experimental.chat.system.transform input does not carry the agent name.
...
export default { id: "workspace-plugin", server: WorkspacePlugin }
