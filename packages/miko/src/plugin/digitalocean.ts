import type { Hooks, PluginInput } from "@miko-ai/plugin"
import type { Model } from "@miko-ai/sdk/v2"
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>Miko - DigitalOcean Authorization</title>
    <style>
    ...
    <script>
      (async function() {
        ...
        try {
          ...
          msgEl.textContent = "You can close this window and return to Miko."
...
async function listRouters(
  ...
  const res = await fetch(`${DO_GENAI_API}/models/routers`, {
    headers: {
      ...
      "User-Agent": `miko/${InstallationVersion}`,
    },
      ...
      methods: [
        {
          ...
          async authorize() {
            ...
            return {
              url,
              instructions:
                "Sign in to DigitalOcean in your browser. Miko will use your DigitalOcean API token directly for inference and load your Inference Routers. Re-run /connect to refresh routers later.",
