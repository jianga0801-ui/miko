...
export const DEPRECATED_PLUGIN_PACKAGES = ["miko-openai-codex-auth", "miko-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, mikoVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(mikoVersion) || semver.major(mikoVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.miko
  if (typeof range !== "string") return
  if (!semver.satisfies(mikoVersion, range)) {
    throw new Error(`Plugin requires miko ${range} but running ${mikoVersion}`)
  }
}
