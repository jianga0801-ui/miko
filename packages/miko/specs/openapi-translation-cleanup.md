...
Trim `packages/miko/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/miko/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/miko`.
- `[x]` `bun typecheck` from `packages/miko`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/miko/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/miko`.
- `[x]` `bun dev generate > /tmp/miko-openapi.json` from `packages/miko`.
...
- Edit `packages/miko/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/miko-openapi.json` from `packages/miko`.
...
- Check source schemas in files like `packages/miko/src/session/schema.ts`, `packages/miko/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/miko/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/miko/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
