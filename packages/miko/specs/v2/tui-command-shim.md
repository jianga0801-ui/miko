...
Delete `packages/miko/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/miko/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/miko/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/miko`
- TUI plugin loader tests in `packages/miko` if runtime plugin API wiring changed
