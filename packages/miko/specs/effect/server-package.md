...
Practical reference for a future `packages/server` split after the miko
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/miko`.
...
- `packages/miko` - current TUI, CLI, and server host
...
Until enough shared service code lives outside `packages/miko`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/miko`

It should not import `packages/miko` services while `packages/miko`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on miko-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/miko` implementation details.
