# Remove `packages/miko/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/miko/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the miko legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/miko/src/storage/db.ts` are concentrated in 22 source files:

- `packages/miko/src/account/repo.ts`
- `packages/miko/src/cli/cmd/db.ts`
- `packages/miko/src/cli/cmd/import.ts`
- `packages/miko/src/cli/cmd/stats.ts`
- `packages/miko/src/control-plane/workspace.ts`
- `packages/miko/src/index.ts`
- `packages/miko/src/node.ts`
- `packages/miko/src/permission/index.ts`
- `packages/miko/src/project/project.ts`
- `packages/miko/src/server/projectors.ts`
- `packages/miko/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/miko/src/server/shared/fence.ts`
- `packages/miko/src/session/message-v2.ts`
- `packages/miko/src/session/projectors.ts`
- `packages/miko/src/session/prompt.ts`
- `packages/miko/src/session/session.ts`
- `packages/miko/src/session/todo.ts`
- `packages/miko/src/share/share-next.ts`
- `packages/miko/src/storage/db.ts`
- `packages/miko/src/sync/index.ts`
- `packages/miko/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy miko database wrapper; `packages/miko/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the miko wrapper.
...
Status: Completed. `SyncEvent` and the miko projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy miko database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy miko database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy miko database wrapper.
...
All migration groups are complete or superseded. `packages/miko/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No miko data-migration group remains.

The previous miko `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate miko data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/miko`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/miko/src`
- `bun typecheck` from `packages/miko`
- Relevant package tests from `packages/miko`, not the repo root
