...
Both `action` and `resource` use miko's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.miko` files is intentionally deferred until `.miko` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether miko permits an operation through its managed execution paths.
