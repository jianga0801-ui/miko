...
Miko 的 TUI 由一套内置 feature 插件驱动，启动时自动加载、零配置即用。它们与第三方插件共享同一套插件接口（见下文「扩展体系」），你也可以用相同方式编写并安装自己的插件。

| 插件 | 作用 |
| --- | --- |
| `which-key` | 按键提示浮层，实时展示可用快捷键与待输入的按键序列。 |
| `diff-viewer` | 内置 diff 查看器，支持文件树浏览与分栏对比。 |
| `plugin-manager` | 插件管理界面，浏览并启用 / 禁用已安装的插件。 |
| `notifications` | 桌面 / 系统通知，在任务完成或需要关注时提醒你。 |
| `sidebar-context` | 侧栏：当前上下文用量与会话信息。 |
| `sidebar-mcp` | 侧栏：已连接的 MCP Server 及其状态。 |
| `sidebar-lsp` | 侧栏：LSP 服务器状态（默认启用）。 |
| `sidebar-todo` | 侧栏：当前任务 / TODO 列表。 |
| `sidebar-files` | 侧栏：本次会话改动过的文件。 |
| `home-footer`、`home-tips`、`sidebar-footer` | 主屏与侧栏的页脚信息和使用提示。 |

### `/` 命令

Miko 内置了强大且高度可定制的快捷命令系统。这些命令可以来自 Miko 的内置程序、本地项目配置文件、MCP Server 提供的 Prompts，或是您自己安装的 Skills。
...
所有已加载的 MCP Prompt 和自定义 Skill 都会自动作为高级指令注册到 `/` 命令列表中，为您提供无缝、可自动补全的终端捷径体验。
...
Miko 专为 **小米 Mimo (Xiaomi Mimo)** 大模型家族设计，将其作为首要的、深度优化的第一方 Provider。
...
不改核心代码也能扩展 Miko，完美兼容原 OpenCode 的所有扩展：
...
- **Providers**：可使用内置 AI SDK provider，也可定义自定义 endpoint 和 provider-specific options。
