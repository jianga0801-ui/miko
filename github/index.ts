...
const { client, server } = createMiko()
...
try {
  ...
  console.log("miko session", session.id)
  if (shareId) {
...
function createMiko() {
  ...
  const proc = spawn(`miko`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createMikoClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/ob|\/miko|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/ob`, `/miko` or `/oc`")
  }
}
...
async function assertMikoConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to miko server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.miko.dev" : "https://miko.dev"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.miko.dev/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("miko-github-action")
    response = await fetch("https://api.miko.dev/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/ob" || body === "/miko" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/ob") || body.includes("/miko") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to miko...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "miko-agent[bot]"`
  await $`git config --global user.email "miko-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `miko/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/miko-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[miko session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
