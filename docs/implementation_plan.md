...
Miko is a TypeScript/Bun fork of Miko. The runtime is centered on `packages/miko` as the CLI/TUI/server shell, shared provider/config state lives in `packages/core`, native LLM protocols live in `packages/llm`, and app/visual surfaces live in `packages/app` and `packages/ui`. 
